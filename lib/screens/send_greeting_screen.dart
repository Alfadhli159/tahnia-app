...
class _SendGreetingScreenState extends State<SendGreetingScreen> {
  ...
  final _openAIService = OpenAIService(apiKey: 'YOUR_API_KEY');

  Future<void> _generateGreetingWithAI() async {
    ...
    try {
      final greetingText = await _openAIService.generateGreeting(
        occasion: _selectedOccasion,
        recipient: _recipientController.text,
        sender: _senderController.text,
        additionalInfo: _selectedRelationship?.toString(),
      );

      setState(() {
        _currentGreeting = Greeting(
          text: greetingText,
          mediaUrl: null, // يمكن إضافة دعم لصور من OpenAI إذا أردت
        );
      });
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('فشل في توليد الرسالة: $e')),
      );
    } finally {
      setState(() {
        _isLoading = false;
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              // زر توليد الرسالة باستخدام OpenAI
              AnimatedButton(
                onPressed: _isLoading ? null : _generateGreetingWithAI,
                child: Row(
